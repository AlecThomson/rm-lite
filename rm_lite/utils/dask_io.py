...
def zarr_store_layout(
    ...
    band read out of the FITS cube, and a task holds that band about three times
    over: the raw read, zarr's encode buffer, and a copy between them. Hence the
    third of the target.
    """
    shard_rows, _ = spatial_chunk_size(
        ...
        nx=nx,
        itemsize=itemsize,
        target_chunk_mb=target_chunk_mb / 3,
    )
